...
#define PROGRAM_NAME "Octree-GS Viewer"
...
int main(int ac, char** av) 
{
	...
	int sh_degree = 3;
	int fork = 2;
	rng = findArg(cfgLine, "fork");
	fork = std::stoi(cfgLine.substr(rng.first, rng.second - rng.first));
	...
	while (window.isOpened()) 
	{
		...
		if (1.0 * freeMemory / totalMemory < 0.2)
